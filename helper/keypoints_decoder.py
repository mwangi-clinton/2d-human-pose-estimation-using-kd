...
def _calc_distances(
    preds: np.ndarray, targets: np.ndarray, mask: np.ndarray, normalize: np.ndarray
) -> np.ndarray:
...
def _distance_acc(distances: np.ndarray, thr: float = 0.5) -> Any:
...
def _get_max_preds_3d(heatmaps: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
...
def keypoint_nme(
    ...
    return float(distance_valid.sum() / max(1, len(distance_valid)))
...
def post_dark_udp(
    coords: np.ndarray,
    batch_heatmaps: Union[np.ndarray, "torch.Tensor"],
    kernel: int = 3,
) -> np.ndarray:
...
def keypoints_from_regression(
    regression_preds: np.ndarray,
    center: np.ndarray,
    scale: np.ndarray,
    img_size: list | np.ndarray,
...
def keypoints_from_heatmaps(
    ...
    scale: np.ndarray,
    unbiased: bool = False,
    post_process: str | bool | None = "default",
    kernel: int = 11,
...
def keypoints_from_heatmaps3d(
    heatmaps: np.ndarray, center: np.ndarray, scale: np.ndarray
) -> Tuple[np.ndarray, np.ndarray]:
...
def get_centers_and_scales_xyxy(
    person_boxes: List[List[float]], scale_factor: float = 1.0
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Compute centers and scales for person bounding boxes in XYXY format.

    Args:
        person_boxes (list of list): Each box is [x1, y1, x2, y2].
        scale_factor (float): Optional factor to enlarge the scale
                              (default = 1.0, e.g. use 1.25 for padding).

    Returns:
        tuple: (centers, scales)
            centers (np.ndarray[N, 2]): center coordinates [x, y]
            scales (np.ndarray[N, 2]): scale values [scale_x, scale_y]
    """
    centers = []
    scales = []
    ...
    for box in person_boxes:
        ...
        w = x2 - x1
        h = y2 - y1
        scale_x = (w / 200.0) * scale_factor
        scale_y = (h / 200.0) * scale_factor
        scales.append([scale_x, scale_y])

    return np.array(centers), np.array(scales)


def get_centers_and_scales(
    person_boxes: List[List[float]], scale_factor: float = 1.0
) -> Tuple[np.ndarray, np.ndarray]:
    """
    Compute centers and scales for person bounding boxes in COCO format.

    Args:
        person_boxes (list of list): Each box is [x, y, w, h].
        scale_factor (float): Optional factor to enlarge the scale
                              (default = 1.0, e.g. use 1.25 for padding).
    ...
    for box in person_boxes:
        ...
        # Compute scale (normalized, with optional padding)
        scale_x = (w / 200.0) * scale_factor
        scale_y = (h / 200.0) * scale_factor
        scales.append([scale_x, scale_y])
